...
def generate_pdf(data):
    ...
    if comp:
        ...
        crow = [
            ...
            ["TF-IDF Similarity", str(comp.get("similarity", "-")),     "40%"],
            ["Keyword Coverage",  str(comp.get("keywords",   "-")),     "25%"],
            ["Grammar & Spelling",str(comp.get("grammar",    "-")),     "20%"],
            ["Answer Length",     str(comp.get("length",     "-")),     "15%"],
        ]
    ...
    for s in data.get("strengths", []):
        el.append(Paragraph("  + " + s, S["body"]))
    el.append(Spacer(1, 0.1*cm))
    for w in data.get("weaknesses", []):
        el.append(Paragraph("  -> " + w, S["body"]))
    el.append(Spacer(1, 0.3*cm))
    ...
    for i, r in enumerate(data.get("results", [])):
        ...
        el.append(Paragraph("Q" + str(i + 1) + ". " + question, S["qhead"]))
        el.append(Paragraph(
            "Score: " + str(score) + " / 10",
            ParagraphStyle("sc", parent=S["bold"], textColor=sc_color(score))
        ))
        el.append(Paragraph("<b>Your Answer:</b> "  + ua, S["body"]))
        el.append(Paragraph("<b>Model Answer:</b> " + ma, S["body"]))

        mk = r.get("missing_keywords", [])
        if mk:
            el.append(Paragraph("<b>Missing Keywords:</b> " + ", ".join(mk), S["small"]))

        gs = r.get("grammar_suggestions", [])
        if gs:
            el.append(Paragraph("<b>Grammar:</b> " + "; ".join(gs), S["small"]))

        fb = r.get("feedback", "")
        if fb:
            el.append(Paragraph("<i>" + fb + "</i>", S["small"]))
    ...
    el.append(Paragraph(
        "Generated by iReady Lite — Practice. Analyze. Improve.",
        S["small"]
    ))
